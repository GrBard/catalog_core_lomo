...
from customtkinter import CTkFrame, CTkButton, CTkCheckBox, CTkLabel, CTkScrollableFrame, CTkImage
...
class AppUI:
    def __init__(self, root, file_manager):
        ...
        self.btn_select_samples = CTkButton(self.top_frame, text="Файл с образцами...", command=self.select_samples_file,
        ...
        # Фрейм для таблицы
        self.table_frame = CTkFrame(self.main_frame, corner_radius=10)
    ...
    def process_data(self):
        ...
        try:
            ...
            messagebox.showinfo("Успех", f"Обработано {len(df)} записей!")
    ...
    def save_data(self):
        """Сохраняет текущий DataFrame в файл."""
        ...
        try:
            ...
            if result_path:
                ...
                messagebox.showinfo("Успех", f"Данные сохранены в {result_path}")
        except Exception as e:
            messagebox.showerror("Ошибка сохранения", str(e))

    def create_catalog(self):
        """Создаёт каталог в формате Word."""
        if not self.data_processor or self.data_processor.get_current_dataframe() is None:
        ...
        try:
            ...
            if save_path:
                self.data_processor.create_catalog(save_path)
                self.status_var.set(f"Каталог сохранён: {save_path}")
                # Показываем уведомление с вопросом об открытии
                response = messagebox.askyesno("Успех", f"Каталог сохранён: {save_path}\nХотите открыть его?")
    ...
    def preview_catalog(self):
        ...
        # Заголовок
        title_label = CTkLabel(preview_frame, text=f"Фотографии керна по скважине {self.data_processor.get_current_dataframe()['Скважина'].iloc[0]}",
                               font=("Helvetica", 18, "bold"))
        title_label.pack(pady=10)

        # Вводный текст
        intro_label = CTkLabel(preview_frame, text="Глубины даны по керну. Номера образцов расположены напротив точек выбуривания. "
        ...
        scale_image_path = os.path.join(os.path.dirname(__file__), 'resources', 'scale.jpg')
        shkala_image_path = os.path.join(os.path.dirname(__file__), 'resources', 'shkala.jpg')
